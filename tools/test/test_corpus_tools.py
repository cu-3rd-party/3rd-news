...
from tools.lib.interactor.use_cases.copy_labels import manual_labels, plan_transfers
from tools.lib.interactor.use_cases.reject_noise import (
    broadcasters,
    find_candidates,
    reasons_to_reject,
)
from tools.lib.interactor.use_cases.sample import candidates, pick, quotas
...
def test_pinned_finds_only_posts_with_a_manual_label_on_that_facet():
    from tools.lib.interactor.use_cases.release_facet import pinned
...
def test_progress_counts_manual_labels_and_ignores_source_driven_axes():
    from tools.lib.interactor.use_cases.progress import human_facets, labelled
...
def test_progress_report_says_how_much_is_done():
    from tools.lib.interactor.use_cases.progress import report

    full = ["topic", "action", "importance", "audience"]
    items = [
        {**item("a1", ANNOUNCEMENT, channel="swe"), "manual_facets": full},
        {
            **item("a2", "Экзамен 15 июня, устно, в кампусе", channel="ai"),
            "manual_facets": ["topic"],
        },
...
def test_gold_lists_only_frozen_posts():
    from tools.lib.interactor.use_cases.gold import describe, golden
