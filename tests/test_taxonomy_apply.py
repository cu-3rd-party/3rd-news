"""Планировщик `tools/taxonomy/apply.py` и валидность самого `taxonomy.json`."""
...
from tools.taxonomy.apply import TAXONOMY_PATH, plan_changes, plan_sources, source_defaults
...
def test_position_comes_from_the_file_so_the_screen_matches_the_guideline():
    """Порядок осей — часть инструкции: по нему разметчик жмёт цифры."""

    spec = DESIRED[1]  # action, position 1
    existing = [
        {
            "id": "f1",
            "slug": "action",
            ...
            "type": spec["type"],
            "ai_hint": spec["ai_hint"],
            "position": 0,  # осталось от стартовой заглушки
            "values": [],
        }
    ]
...
def test_patch_sends_the_whole_record_because_patch_replaces_it():
    """PATCH в API — полная замена, поэтому в теле должны быть и нетронутые поля."""
    ...
    assert payload["position"] == spec["position"]  # из файла
    assert payload["required"] is True  # чужое поле сохранилось
...
def test_unknown_facets_and_values_are_only_deactivated_never_deleted():
    existing = [
        {"id": "f9", "slug": "stream", "title": "Поток", "type": "multi", "values": []},
        {
            ...
            "values": [{"id": "v9", "slug": "legacy", "title": "Старое", "synonyms": [], "match_patterns": []}],
        },
    ]
    ...
    assert plan.deactivate_facets[0].payload["title"] == "Поток"  # запись не теряется
    ...
    for spec in DESIRED:
        ...
        for value in spec["values"]:
            ...
            for pattern in value["match_patterns"]:
                re.compile(pattern)  # классификатор молча глотает битые — ловим здесь


def test_axes_match_the_annotation_guideline():
    guideline = Path("docs/annotation-guideline.md").read_text(encoding="utf-8")
    for spec in DESIRED:
        for value in spec["values"]:
...
def test_program_is_filled_from_channels_not_from_text():
    """`program` не угадывается по словам: у значений нет ни синонимов, ни паттернов."""
...
def test_plan_sources_patches_only_what_differs_and_keeps_other_facets():
    existing = [
        ...
        {"id": "s2", "slug": "swe", "title": "Разработка", "default_labels": {"program": ["swe-2025"]}},
    ]
    wanted = {"ai": {"program": ["ai-2025"]}, "swe": {"program": ["swe-2025"]}, "gone": {"program": ["x"]}}

    patches, missing = plan_sources(existing, wanted)

    assert [(p.id, p.label) for p in patches] == [("s1", "ai")]
    assert patches[0].changed == {"default_labels": {"importance": ["useful"], "program": ["ai-2025"]}}
    assert missing == ["gone"]


def test_source_patch_carries_the_whole_record():
    """`update_source` тоже перезаписывает объект и требует `title`."""
