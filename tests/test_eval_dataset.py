"""Загрузка золотого набора и таксономии в измеритель."""
...
from tools.eval.dataset import gold_labels, load_context, load_records, load_taxonomy
...
def test_taxonomy_from_admin_facets_list():
    ...
    assert taxonomy.facet("stream").type.value == "multi"
    assert taxonomy.facet("importance").values[0].synonyms == ["срочно", "дедлайн"]
