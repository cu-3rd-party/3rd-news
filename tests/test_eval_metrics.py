"""Метрики по осям: точность, F1 по значениям, калибровка."""
...
from tools.eval.metrics import calibration, facet_metrics, summarize
from tools.eval.runners import Prediction
...
def test_summarize_skips_untouched_facets_and_aggregates_cost():
    ...
    assert stream["n"] == 4  # n4 не трогал ось stream
    assert summary["n"] == 5
