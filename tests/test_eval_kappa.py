"""Согласие разметчиков: слепая выгрузка и каппа Коэна."""
...
from tools.eval.kappa import cohen_kappa, kappa_report, read_labels_csv, write_blind_csv
...
def test_kappa_report_lists_disagreements():
    ...
    other = {
        ...
        "n2": {"importance": {"critical"}, "stream": set()},  # расходится по importance
        "n3": {"importance": {"critical"}, "stream": {"2026"}},
    }
    report = kappa_report(records, other, taxonomy)
    importance = report["importance"]
    assert importance["n"] == 3
    assert importance["disagreements"] == [
        {"id": "n2", "gold": ["normal"], "other": ["critical"]}
    ]
