"""Прогон классификаторов в измерителе — без HTTP и без ядра."""
...
from tools.eval.dataset import load_records, load_taxonomy
from tools.eval.runners import (
    Prediction,
    build_request,
    combine,
    load_classifiers,
...
def test_regex_runner_finds_keywords(modules):
    ...
    # «срочно» + «дедлайн» — два совпадения: 0.6 + 0.1.
    assert prediction.labels["importance"] == [("critical", pytest.approx(0.7))]
    # Синоним «второй курс» на «второго курса» не ложится: regex ищет начало
    # слова и точную форму. Это известное ограничение словаря, не баг теста.
...
def test_ai_runner_uses_cache_instead_of_network(modules, tmp_path):
    ...
    content = json.dumps(
        {"labels": [{"facet": "importance", "value": "critical", "confidence": 0.9}]}
    )
    body = {
...
def test_combine_lets_regex_outrank_ai_and_applies_thresholds():
    ...
    assert merged.labels["importance"] == [("normal", 0.6)]  # regex сильнее по той же оси
    assert merged.labels["stream"] == [("2025", 0.8)]  # 0.4 ниже порога
