...
- `apps/web`: существующие сценарии админки, перенесённые без редизайна на
  актуальные React 19, Vite 8 и TypeScript 7. Рядом со сборкой лежит
  `public/preview.html` — отдельная страница `/preview`, которая показывает
  ленту глазами клиента: спрашивает ключ на чтение и ходит только в
  `/api/v1/feed` и `/api/v1/taxonomy`.
